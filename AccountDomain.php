...
class AccountDomain
{
    ...
    public const skinEnhanced = 'enhanced';
    public const skinDefault = 'default';
    public const skinPowerUser = 'power_user';
    public const skinEvolution = 'evolution';
    ...
    public function reload()
    {
        ...
        if (isset($result['error']) and 1 == $result['error']) {
            if ('Unable to show user' == $result['text']) {
        ...
        if (isset($usage['error']) and 1 == $usage['error']) {
            $exception = new FailedException();
            $exception->setRequest($params);
        ...
        $this->maxQuota = 'unlimited' != $result['quota'] ? $result['quota'] : self::unlimited;
        $this->maxBandwidth = 'unlimited' != $result['bandwidth'] ? $result['bandwidth'] : self::unlimited;
        $this->maxEmails = 'unlimited' != $result['nemails'] ? $result['nemails'] : self::unlimited;
        $this->maxFtps = 'unlimited' != $result['ftp'] ? $result['ftp'] : self::unlimited;
        $this->maxAddonDomains = 'unlimited' != $result['vdomains'] ? $result['vdomains'] : self::unlimited;
        $this->maxParkDomains = 'unlimited' != $result['domainptr'] ? $result['domainptr'] : self::unlimited;
        $this->maxSubDomains = 'unlimited' != $result['nsubdomains'] ? $result['nsubdomains'] : self::unlimited;
        $this->maxSqls = 'unlimited' != $result['mysql'] ? $result['mysql'] : self::unlimited;
        $this->maxEmailForwarders = 'unlimited' != $result['nemailf'] ? $result['nemailf'] : self::unlimited;
        $this->maxEmailResponders = 'unlimited' != $result['nemailr'] ? $result['nemailr'] : self::unlimited;
        $this->reseller = ('reseller' == $result['usertype']);
        $this->anonymousFtp = ('ON' == $result['aftp']);
        $this->php = ('ON' == $result['php']);
        $this->spam = ('ON' == $result['spam']);
        $this->cron = ('ON' == $result['cron']);
        $this->sysinfo = ('ON' == $result['sysinfo']);
        $this->ssh = ('ON' == $result['ssh']);
        $this->ssl = ('ON' == $result['ssl']);
        $this->dnscontrol = ('ON' == $result['dnscontrol']);
        $this->cgi = ('ON' == $result['cgi']);
        $this->skin = $result['skin'];
        $this->package = $result['package'];
        $this->suspended = ('yes' == $result['suspended']);
        $this->ip = $result['ip'];
        ...
        if (isset($result['creator']) and 'root' != $result['creator'] and 'admin' != $result['creator'] and $result['creator'] != $oldUsername) {
    ...
    public function delete()
    {
        ...
        if (isset($result['error']) and 1 == $result['error']) {
            if ('Error while deleting Users' == $result['text']) {
                $exception = new NotFoundAccountException($this->username);
            } else {
                $exception = new FailedException();
            }
    ...
    public function restore(string $file, ?string $ip = null, int $timeout = 1200): Account
    {
        ...
        $checkUserInTicket = function (string $message): bool {
            return false !== stripos($message, "User {$this->username} has been restored") || false !== stripos($message, $this->username);
        };
        while ((0 == $timeout or Date::time() - $startAt < $timeout) and !$found) {
            $tickets = $this->getTickets();
            foreach ($tickets as $ticket) {
                if (!$lastTicket or $ticket['last_message'] > $lastTicket['last_message']) {
                    ...
                    if (
                        'your user files have been restored' == substr($subject, 0, 34)
                        and 'yes' == strtolower($ticket['new'])
                    ) {
                        ...
                        'an error occurred during the restore' == substr($subject, 0, 36)
                        and 'yes' == strtolower($ticket['new'])
                    ) {
                        $content = $this->getTicket($ticket['message']);
                        if ($content) {
        ...
        while ((0 == $timeout or Date::time() - $startAt < $timeout) and !$found) {
            ...
            foreach ($tickets as $ticket) {
                if (!$lastTicket or $ticket['last_message'] > $lastTicket['last_message']) {
                    ...
                    if (
                        'your backups are now ready' == substr($subject, 0, strlen('your backups are now ready'))
                        and 'yes' == strtolower($ticket['new'])
                    ) {
                        $content = $this->getTicket($ticket['message']);
                        if ($content) {
                            if (false !== stripos($content['message'], "User {$this->username} has been backed up")) {
                        ...
                        'an error occurred during the backup' == substr($subject, 0, strlen('an error occurred during the backup'))
                        and 'yes' == strtolower($ticket['new'])
                    ) {
                        $log->reply("Oh it's seems an error occurred, Let's check it");
                        $content = $this->getTicket($ticket['message']);
                        if ($content) {
                            if (false !== stripos($content['message'], "User {$this->username} has been backed up") or false !== stripos($content['message'], "{$this->username}.tar.gz")) {
    ...
    public function modify(array $modifiedData)
    {
        ...
        if (isset($modifiedData['maxQuota'])) {
            if (self::unlimited == $modifiedData['maxQuota']) {
                $params['uquota'] = 'ON';
            } else {
                $params['quota'] = $modifiedData['maxQuota'];
            }
        } elseif (self::unlimited != $this->maxQuota) {
            $params['quota'] = $this->maxQuota;
        } else {
            $params['uquota'] = 'ON';
        }
        if (isset($modifiedData['maxBandwidth'])) {
            if (self::unlimited == $modifiedData['maxBandwidth']) {
                $params['ubandwidth'] = 'ON';
            } else {
                $params['bandwidth'] = $modifiedData['maxBandwidth'];
            }
        } elseif (self::unlimited != $this->maxBandwidth) {
            $params['bandwidth'] = $this->maxBandwidth;
        } else {
            $params['ubandwidth'] = 'ON';
        }
        if (isset($modifiedData['maxAddonDomains'])) {
            if (self::unlimited == $modifiedData['maxAddonDomains']) {
                $params['uvdomains'] = 'ON';
            } else {
                $params['vdomains'] = $modifiedData['maxAddonDomains'];
            }
        } elseif (self::unlimited != $this->maxAddonDomains) {
            $params['vdomains'] = $this->maxAddonDomains;
        } else {
            $params['uvdomains'] = 'ON';
        }
        if (isset($modifiedData['maxParkDomains'])) {
            if (self::unlimited == $modifiedData['maxParkDomains']) {
                $params['udomainptr'] = 'ON';
            } else {
                $params['domainptr'] = $modifiedData['maxParkDomains'];
            }
        } elseif (self::unlimited != $this->maxParkDomains) {
            $params['domainptr'] = $this->maxParkDomains;
        } else {
            $params['udomainptr'] = 'ON';
        }
        if (isset($modifiedData['maxSubDomains'])) {
            if (self::unlimited == $modifiedData['maxSubDomains']) {
                $params['unsubdomains'] = 'ON';
            } else {
                $params['nsubdomains'] = $modifiedData['maxSubDomains'];
            }
        } elseif (self::unlimited != $this->maxSubDomains) {
            $params['nsubdomains'] = $this->maxSubDomains;
        } else {
            $params['unsubdomains'] = 'ON';
        }
        if (isset($modifiedData['maxEmails'])) {
            if (self::unlimited == $modifiedData['maxEmails']) {
                $params['unemails'] = 'ON';
            } else {
                $params['nemails'] = $modifiedData['maxEmails'];
            }
        } elseif (self::unlimited != $this->maxEmails) {
            $params['nemails'] = $this->maxEmails;
        } else {
            $params['unemails'] = 'ON';
        }
        if (isset($modifiedData['maxEmailForwarders'])) {
            if (self::unlimited == $modifiedData['maxEmailForwarders']) {
                $params['uunemailf'] = 'ON';
            } else {
                $params['nemailf'] = $modifiedData['maxEmailForwarders'];
            }
        } elseif (self::unlimited != $this->maxEmailForwarders) {
            $params['unemailf'] = $this->maxEmailForwarders;
        } else {
            $params['uunemailf'] = 'ON';
        }
        if (isset($modifiedData['maxEmailResponders'])) {
            if (self::unlimited == $modifiedData['maxEmailResponders']) {
                $params['unemailr'] = 'ON';
            } else {
                $params['nemailr'] = $modifiedData['maxEmailResponders'];
            }
        } elseif (self::unlimited != $this->maxEmailResponders) {
            $params['nemailr'] = $this->maxEmailResponders;
        } else {
            $params['unemailr'] = 'ON';
        }
        if (isset($modifiedData['maxMailingLists'])) {
            if (self::unlimited == $modifiedData['maxMailingLists']) {
                $params['unemailml'] = 'ON';
            } else {
                $params['nemailml'] = $modifiedData['maxMailingLists'];
            }
        } elseif (self::unlimited != $this->maxMailingLists) {
            $params['nemailml'] = $this->maxMailingLists;
        } else {
            $params['unemailml'] = 'ON';
        }
        if (isset($modifiedData['maxSqls'])) {
            if (self::unlimited == $modifiedData['maxSqls']) {
                $params['umysql'] = 'ON';
            } else {
                $params['mysql'] = $modifiedData['maxSqls'];
            }
        } elseif (self::unlimited != $this->maxSqls) {
            $params['mysql'] = $this->maxSqls;
        } else {
            $params['umysql'] = 'ON';
        }
        if (isset($modifiedData['maxFtps'])) {
            if (self::unlimited == $modifiedData['maxFtps']) {
                $params['uftp'] = 'ON';
            } else {
                $params['ftp'] = $modifiedData['maxFtps'];
            }
        } elseif (self::unlimited != $this->maxFtps) {
            $params['ftp'] = $this->maxFtps;
        } else {
            $params['uftp'] = 'ON';
        }
    ...
    {
        if ($maxEmailForwarders < 0 and self::unlimited != $maxEmailForwarders) {
    ...
    {
        if ($maxEmailResponders < 0 and self::unlimited != $maxEmailResponders) {
    ...
    {
        if ($maxQuota <= 0 and self::unlimited != $maxQuota) {
    ...
    {
        if ($maxBandwidth <= 0 and self::unlimited != $maxBandwidth) {
            throw new Exception();
        }
    ...
    {
        if ($maxFtps < 0 and self::unlimited != $maxFtps) {
    ...
    {
        if ($maxEmails < 0 and self::unlimited != $maxEmails) {
    ...
    {
        if ($maxMailingLists < 0 and self::unlimited != $maxMailingLists) {
    ...
    {
        if ($maxSqls < 0 and self::unlimited != $maxSqls) {
    ...
    {
        if ($maxSubDomains < 0 and self::unlimited != $maxSubDomains) {
    ...
    {
        if ($maxParkDomains < 0 and self::unlimited != $maxParkDomains) {
    ...
    {
        if ($maxAddonDomains < 0 and self::unlimited != $maxAddonDomains) {
