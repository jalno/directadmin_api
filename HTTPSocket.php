...
class HTTPSocket
{
    ...
    public function query($request, $content = '', $doSpeedCheck = 0)
    {
        ...
        $array_headers = [
            'Host' => (80 == $this->remote_port ? $this->remote_host : "$this->remote_host:$this->remote_port"),
            'Accept' => '*/*',
            'Connection' => 'Close'];
        ...
        if (isset($this->remote_uname) && null == $this->remote_passwd) {
        ...
        $this->isJson = (isset($headers['content-type']) and false !== stripos($headers['content-type'], 'json'));
        ...
        if ($this->doFollowLocationHeader) {
            ...
            if (isset($headers['x-use-https']) && 'yes' == $headers['x-use-https']) {
